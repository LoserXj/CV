...
def get_tiny_images(image_paths):
    ...
    for path in image_paths:
        ...
        # 对向量进行归一化处理，使其均值为0，长度为1
        normalized_image = vectorized_image - np.mean(vectorized_image)
        normalized_image = normalized_image / np.linalg.norm(normalized_image)

        feats.append(normalized_image)
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats,k=7,
                              metric='euclidean'):
    """
    ...
    """
    test_labels = []
    for test_feats in test_image_feats:
        # 计算测试图像特征与所有训练图像特征之间的距离
        distances = pairwise_distances(test_feats.reshape(1, -1), train_image_feats, metric=metric)
        
        # 找到最近的k个邻居的索引
        nearest_neighbor_indices = np.argsort(distances)[0][:k]
        
        # 根据最近的邻居的标签进行投票
        votes = [train_labels[i] for i in nearest_neighbor_indices]
